#!/usr/bin/env python3
"""
Train CTC+SAC Agent in CoppeliaSim
====================================
CoDIT 2026 Paper: CTC + SAC Hybrid Control

Usage (inside Docker):
    cd /workspace/src
    python train_ctc_sac.py

Saves best model to: ../models/best_ctc_sac.pt
...
def setup_joints(sim):
    """
    Lock ALL Mico joints at zero, then enable torque mode only on
    shoulder (j1) and elbow (j2).  Returns j1, j2, all_joint_handles.
    """
    mico       = sim.getObject('/Mico')
    all_joints = sim.getObjectsInTree(mico, sim.object_joint_type, 0)
    for jh in all_joints:
        sim.setObjectInt32Param(jh, sim.jointintparam_dynctrlmode,
                                sim.jointdynctrl_position)
        sim.setJointTargetPosition(jh, 0.0)
    j1 = sim.getObject('/Mico/joint/link/joint')             # shoulder (handle 21)
    j2 = sim.getObject('/Mico/joint/link/joint/link/joint')  # elbow   (handle 23)
...
def reset_episode(sim, j1, j2, all_joints):
    """Lock all joints at 0, re-enable torque on shoulder+elbow."""
...
def main():
    print("=" * 60)
    print("  CTC+SAC Training — CoDIT 2026")
    print("=" * 60)
    ...
    # Lock all 6 joints, then enable torque only on shoulder + elbow
    j1, j2, all_joints = setup_joints(sim)
    print(f"✅ All 6 joints found — joints 4,5,6 locked; shoulder+elbow in torque mode")
    ...
    for ep in range(NUM_EPISODES):
        ...
        if ep_reward > best_reward:
            ...
            agent.save(f"{MODELS_DIR}/best_ctc_sac.pt")
        ...
        if (ep + 1) % SAVE_EVERY == 0:
            agent.save(f"{MODELS_DIR}/checkpoint_ep{ep+1}.pt")

    agent.save(f"{MODELS_DIR}/final_ctc_sac.pt")
    json.dump({'rewards': rewards, 'errors': errors},
              open(f"{RESULTS_DIR}/training_ctc_sac.json", 'w'), indent=2)
    print("\n✅ Training complete!")
    print(f"   Best model → {MODELS_DIR}/best_ctc_sac.pt")
