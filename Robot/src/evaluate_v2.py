#!/usr/bin/env python3
"""
Evaluate All Methods — CTC-Only, CTC+SAC, RL-Only, PID+SAC  (v2 — 2-DOF scene)
=================================================================================
...
Changes from v1:
  ...
  - Loads *_v2.pt models
  - CTC+SAC scale updated to 10.0 (matching v2 training range)
  - Saves to comparison_v2.json and table_vi_v2.txt
...
Outputs:
    ../results/comparison_v2.json     — all metrics
    ../results/table_vi_v2.txt        — ready to paste into paper
    ../results/ts_v2_<method>.csv     — raw time-series per method
...
DT               = 0.01   # 10 ms step
SS_FRAC          = 0.5    # Steady-state = last 50% (t ≥ 5s)
TR_FRAC          = 0.2    # Transient    = first 20% (t < 2s)
MODELS_DIR       = "../models"
RESULTS_DIR      = "../results"
...
def load_actor(path, scale):
    ...
    if isinstance(ckpt, dict):
        weights = (ckpt.get('actor') or ckpt.get('actor_state_dict') or ckpt)
    else:
        weights = ckpt
...
# ── RUN ONE METHOD ────────────────────────────────────────────────
def run(sim, j1, j2, ctc, pid, ctc_sac_actor, rl_only_actor, pid_sac_actor, method):
    print(f"\n{'='*55}\n  {method}\n{'='*55}")

    reset_eval(sim, j1, j2)
    if pid is not None:
        pid.reset()
    ...
    while t < DURATION:
        ...
        if method == "CTC-Only":
            ...
            tau = np.clip(tau_ctc + np.clip(tau_rl, -10.0, 10.0), -20.0, 20.0)
    ...
    with open(f"{RESULTS_DIR}/ts_v2_{tag}.csv", 'w', newline='') as f:
        w = csv.writer(f)
...
def print_table(all_m):
    ...
    sep    = "-" * len(header)
    print(f"\n\n{'='*len(header)}")
    print("  UPDATED TABLE VI — CoDIT 2026  (v2 results)")
    print(f"{'='*len(header)}")
...
def main():
    print("=" * 60)
    print("  Evaluation v2 — CoDIT 2026  (2-DOF scene)")
    print("=" * 60)
    ...
    print("✅ 2 joints — shoulder (/Mico/joint) + elbow (/Mico/joint/link/joint)")
    ...
    # Load v2 models — fall back to v1 models if v2 not found yet
    ctc_sac_actor = rl_only_actor = pid_sac_actor = None

    for p, scale in [
        (f"{MODELS_DIR}/best_ctc_sac_v2.pt", 10.0),
        (f"{MODELS_DIR}/best_ctc_sac.pt",     5.0),
    ]:
        if os.path.exists(p):
            ctc_sac_actor = load_actor(p, scale)
            print(f"✅ CTC+SAC model ← {p}  (scale={scale})"); break
    if not ctc_sac_actor:
        print("⚠️  CTC+SAC model not found — skipping")

    for p, scale in [
        (f"{MODELS_DIR}/best_rl_only_v2.pt", 20.0),
    ...
    for p, scale in [
        (f"{MODELS_DIR}/best_pid_sac_v2.pt", 10.0),
    ...
    for method in methods:
        m = run(sim, j1, j2, ctc, pid, ctc_sac_actor, rl_only_actor, pid_sac_actor, method)
    ...
    json.dump(out, open(f"{RESULTS_DIR}/comparison_v2.json", 'w'), indent=2)
    print(f"✅ Saved → {RESULTS_DIR}/comparison_v2.json")

    with open(f"{RESULTS_DIR}/table_vi_v2.txt", 'w') as f:
        f.write("UPDATED TABLE VI — CoDIT 2026 Paper  (v2 — 2-DOF scene)\n")
        ...
        f.write(f"TR Error = mean error over first {int(TR_FRAC*100)}% of episode (t < {DURATION*TR_FRAC:.0f}s)\n")
    print(f"✅ Saved → {RESULTS_DIR}/table_vi_v2.txt")
