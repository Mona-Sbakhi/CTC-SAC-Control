#!/usr/bin/env python3
"""
Train RL-Only (SAC without CTC) — Baseline  (v2 — 2-DOF scene)
================================================================
...
Usage (inside Docker):
    cd /workspace/src
    python train_rl_only_v2.py

Saves best model to: ../models/best_rl_only_v2.pt
...
def main():
    print("=" * 60)
    print("  RL-Only Training v2 — CoDIT 2026")
    print("  (2-DOF scene, action range ±20 N·m)")
    print("=" * 60)
    ...
    print("✅ SAC agent initialised — action range: ±20 N·m")
    ...
    for ep in range(NUM_EPISODES):
        ...
        if ep_reward > best_reward:
            ...
            agent.save(f"{MODELS_DIR}/best_rl_only_v2.pt")
        ...
        if (ep + 1) % SAVE_EVERY == 0:
            agent.save(f"{MODELS_DIR}/rl_only_v2_checkpoint_ep{ep+1}.pt")

    agent.save(f"{MODELS_DIR}/final_rl_only_v2.pt")
    json.dump({'rewards': rewards, 'errors': errors},
              open(f"{RESULTS_DIR}/training_rl_only_v2.json", 'w'), indent=2)
    print("\n✅ RL-Only v2 Training complete!")
    print(f"   Best model → {MODELS_DIR}/best_rl_only_v2.pt")
